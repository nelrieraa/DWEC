...
const cookieVisita = todasLasCookies.split(';').find(c=>c.startsWith('ultimaVisita='));
if(cookieVisita){
    const fechaGuardada = cookieVisita.split('=')[1];
    const banner = document.getElementById('banner-visita');
    banner.innerHTML = `<p>"bienvenido de nuevo. Tu ultima visita fue el"${fechaGuardada}</p>`
    banner.style.display = 'block';
}
const fecha = new Date().toLocaleString();
document.cookie = "ultimaVisita="+ fecha +";max-age= "+(60*60*24*30)+";path=/";
