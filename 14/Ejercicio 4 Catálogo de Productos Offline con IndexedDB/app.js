...
const botonActu = document.getElementById('btn-update');
let db;
...
async function cargarDatos() {
    ...
    obtenerCategorias();
    let tx = db.transaction(['productos'],'readwrite');
    let store = tx.objectStore('productos');
    listaProductos.forEach(p=>store.put(p));
}
...
botonActu.addEventListener('click',function(){
   let tx = db.transaction(['productos'], 'readwrite');
    let store = tx.objectStore('productos');
    

    let solicitudBorrado = store.clear();

    solicitudBorrado.onsuccess = () => {
